...
public:
	inline bool NetworkReady() const { return (this->b && this->b->GetStatus(SF_CONNECTED)); }

	SendQTimer(Bot *bot) : Timer(config->SendQRate, time(NULL), true), sent(0), b(bot)
	{
	...
	void Tick(time_t)
	{
		...
		{
			if(this->b && this->b && this->b->GetStatus(SF_CONNECTED))
				this->b->Write(sqo.SendQ.front());
			else
				Log(LOG_WARN) << "Attempted to send \"" << sqo.SendQ.front() << "\" to the server but no socket exists!";
			sqo.SendQ.pop();
		}
...
IRCProto::IRCProto(Bot *bot) : b(bot)
{
	if(!bot)
		throw CoreException("IRCProto with no bot?");

	this->sqt = new SendQTimer(bot);
}
...
void IRCProto::Raw(const char *fmt, ...)
{
	...
	if(this->b && this->b->GetStatus(SF_CONNECTED))
	{
		this->sqt->SendBuffered(Flux::string(buffer));
	//         this->b->Write(Flux::string(buffer));
	//         this->b->ProcessWrite();
	}
	else
		Log(LOG_WARN) << '[' << this->b->n->name << ']' << " Attempted to send '" << buffer << '\'';
}
/**
...
void IRCProto::notice(const Flux::string &where, const char *fmt, ...)
{
	...
	if(fmt)
	{
		va_start(args, fmt);
		vsnprintf(buffer, sizeof(buffer), fmt, args);
		this->b->notice(where, Flux::string(buffer));
		va_end(args);
	}
}
/**
...
void IRCProto::nick(const char *fmt, ...)
{
	...
	va_start(args, fmt);
	vsnprintf(buffer, sizeof(buffer), fmt, args);
	this->b->nick(Flux::string(buffer));
	va_end(args);
}
/**
...
void IRCProto::introduce_client(const Flux::string &nickname, const Flux::string &ident, const Flux::string &realname)
{
	...
	this->Raw("NICK %s\n", nickname.c_str());
	this->Raw("USER %s %s %s :%s\n", ident.c_str(), hostname, this->b->n->hostname.c_str(), realname.c_str());
