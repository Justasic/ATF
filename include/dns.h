...
struct Question
{
	...
	Question();
	Question(const Flux::string &, QueryType, unsigned short = 1);
};
...
struct ResourceRecord : public Question
{
	...
	ResourceRecord(const Flux::string &, QueryType, unsigned short = 1);
...
/** The request
 */
class DNSRequest : public Timer, public Question
{
 ...
 */
class DNSManager : public Timer, public Socket
{
	typedef std::multimap<Flux::string, ResourceRecord, ci::less> cache_map;
	cache_map cache;
	sockaddrs addrs;
 public:
	...
	DNSManager(const Flux::string &nameserver, int port);
	 ...
	 */
	static DNSQuery BlockingQuery(const Flux::string &mask, QueryType qt);
};
